...
ist_string ist_dumpimage_dump_json(ist_dumpimage* this, ist_strbuf buffer, ist_usize* idxptr) {
    idxptr = idxptr ?: (ist_usize[1]){};
    ...
    for (ist_usize i = 0; i < this->count; ++i) {
        ...
        void*         valp = this->items[i].valp;
        ist_json_kind kind = this->items[i].kind;
...
#define dumps(_vargs...) ist_strbuf_sprintf(buffer, idxptr, ##_vargs)
        ...
        switch (kind) {
            case JKIND_OBJECT:
                ...
                else dumps(fmt, name, ist_dumpimage_dump_json(valp, tbuffer, NULL));
                break;
            case JKIND_ARRAY:
                if (fn) dumps(fmt, name, fn(valp, tbuffer, NULL));
